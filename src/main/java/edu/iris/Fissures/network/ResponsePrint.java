...
public class ResponsePrint {
    ...
    public static String printB53(ChannelId id,
        ...
        if(filter.discriminator().value() != FilterType._POLEZERO)
            return "";
        String transferType = "";
        if(stage.type == edu.iris.Fissures.IfNetwork.TransferType.LAPLACE)
            transferType = "A";
        else if(stage.type == edu.iris.Fissures.IfNetwork.TransferType.ANALOG)
            transferType = "B";
        else if(stage.type == edu.iris.Fissures.IfNetwork.TransferType.COMPOSITE)
            transferType = "C";
    ...
    public static String printB54(ChannelId id,
        ...
        StringBuffer s = new StringBuffer("\n#");
        String transferType = "";
        if(stage.type == edu.iris.Fissures.IfNetwork.TransferType.ANALOG)
            transferType = "A";
        else if(stage.type == edu.iris.Fissures.IfNetwork.TransferType.COMPOSITE)
            transferType = "C";
        else if(stage.type == edu.iris.Fissures.IfNetwork.TransferType.DIGITAL)
    ...
    public static String printB57(ChannelId id,
        ...
        s.append("\nB057F07     Estimated delay (seconds):         "
                + f.format(stage.the_decimation[0].estimated_delay.value));
        s.append("\nB057F08     Correction applied (seconds):      "
                + f.format(stage.the_decimation[0].correction_applied.value));
        s.append("\n#");
        return s.toString();
    }
